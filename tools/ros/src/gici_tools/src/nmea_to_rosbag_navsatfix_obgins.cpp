...
const std::string topic_name = "/gps";
...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < epochs.size(); i++) {
    ...
    msg.status.service =
        sensor_msgs::NavSatStatus::SERVICE_GPS | sensor_msgs::NavSatStatus::SERVICE_GLONASS |
        sensor_msgs::NavSatStatus::SERVICE_GALILEO | sensor_msgs::NavSatStatus::SERVICE_COMPASS;
    if (epoch.sol.stat == SOLQ_FIX) {  // 0.005 m
      msg.status.status = sensor_msgs::NavSatStatus::STATUS_FIX;
      msg.position_covariance[0 + 0 * 3] = square(0.005);
      msg.position_covariance[1 + 1 * 3] = square(0.005);
      msg.position_covariance[2 + 2 * 3] = square(0.005);
    } else {  // 2.0 m
      msg.status.status = sensor_msgs::NavSatStatus::STATUS_NO_FIX;
      msg.position_covariance[0 + 0 * 3] = square(2.0);
      msg.position_covariance[1 + 1 * 3] = square(2.0);
      msg.position_covariance[2 + 2 * 3] = square(2.0);
    }
