                ...
                const std::shared_ptr<DataCluster::GNSS>& gnss)
{
  ...
  float invalid_ephemeris_ratio = (float)num_invalid_ephemeris / 
    (float)(num_invalid_ephemeris + num_valid_ephemeris);
  ...
  const prcopt_t prcopt={ /* defaults processing options */
  ...
  if (role_out == GnssRole::Rover) {
    ...
    epoch.position = Eigen::Map<Eigen::Vector3d>(rtk->sol.rr);
  }
