    ...
    double& depth)
{
  ...
  cv::circle(img_rgb, cv::Point2f(ref_ftr.px(0), ref_ftr.px(1)),
              2, cv::Scalar(0, 0, 255), 5);
  ...
  cv::line(img_rgb, cv::Point2f(px_A(0), px_A(1)),
            cv::Point2f(px_B(0), px_B(1)),
            cv::Scalar(0, 255, 0), 2);
  ...
  cv::line(img_rgb, cv::Point2f(px_A(0), px_A(1)),
            cv::Point2f(px_B(0), px_B(1)),
            cv::Scalar(0, 255, 0), 2);
  cv::circle(img_rgb, cv::Point2f(px_cur_(0), px_cur_(1)),
              2, cv::Scalar(0, 0, 255), 5);
  filename = dataset_dir + "/epipolar_2.png";
  cv::imwrite(filename, img_rgb);
#endif
