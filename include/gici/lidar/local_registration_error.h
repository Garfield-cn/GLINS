...
// Relative point-to-plane registration error between two keyframes
// Parameter blocks: current body pose (7), previous body pose (7)
class LocalRegistrationError
  : public ceres::SizedCostFunction<1 /* number of residuals */,
                                    7 /* parameter blocks 1: current keyframe state */,
                                    7 /* parameter blocks 2: last keyframe state*/>,
...
public:
  ...
  /// \brief Construct with a scan point, plane coefficients, and information
  /// @param[in] params Plane coefficients [nx, ny, nz, d].
  /// @param[in] p Scan point in the LiDAR frame.
  /// @param[in] information Scalar information.
  LocalRegistrationError(const Eigen::Vector4d& params, const Point_lidar p,
                         const Eigen::Matrix<double, 1, 1>& information);

  /// \brief Trivial destructor.
  virtual ~LocalRegistrationError()
  {}

  /// \brief Set the scan point and plane coefficients.
  /// @param[in] params Plane coefficients [nx, ny, nz, d].
  /// @param[in] p Scan point in the LiDAR frame.
  void setMeasurement(const Eigen::Vector4d& params, const Point_lidar p)
  {
    current_point_ = p;
    plane_params_ = params;
  }
  ...
  {
    return ErrorType::kRegistrationError;
  }
