...
public:
  ...
  // Find a match by searching along the epipolar line considering pose error
  MatchResult findEpipolarMatchWithBound(
    const Frame& ref_frame,
    const Frame& cur_frame,
    const FeatureWrapper& ref_ftr,
    const double d_estimate_inv,
    const double d_min_inv,
    const double d_max_inv,
    const double bound_position,
    const double bound_rotation,
    double& depth, 
    Keypoint& image_best,
    int& zmssd_best);
