...
/// \brief Absolute error of roll and pitch angle.
class RollAndPitchError :
    ...
    public ErrorInterface
{
 public:
  ...
  typedef ceres::SizedCostFunction<2, 7> base_t;

  /// \brief The number of residuals (2).
  ...
  RollAndPitchError() = default;

  /// \brief Construct with measurement and information matrix.
  /// @param[in] measurement The roll and pitch angle.
  /// @param[in] information The information (weight) matrix.
  RollAndPitchError(const Eigen::Vector2d& measurement,
           const Eigen::Matrix2d& information);

  /// \brief Trivial destructor.
  virtual ~RollAndPitchError() = default;
  ...
  const Eigen::Vector2d& measurement() const { return measurement_; }
